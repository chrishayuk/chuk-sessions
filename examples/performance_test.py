...
class PerformanceTester:
    ...
    def _get_system_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cpu_freq_ghz": round(psutil.cpu_freq().max / 1000, 1)
            if psutil.cpu_freq()
            else "Unknown",
        }

    async def benchmark_provider(
        self, provider_name: str, operations: int = 1000
    ) -> Dict[str, Any]:
    ...
    async def test_memory_usage(self, provider_name: str) -> Dict[str, Any]:
        ...
        async with session_factory() as session:
            ...
            for batch in [100, 500, 1000, 5000, 10000]:
                ...
                for i in range(batch):
                    await session.setex(
                        f"mem_test_{i}", 300, f"data_{i}" * 10
                    )  # ~70 bytes per item
        ...
        return {"provider": provider_name, "measurements": memory_measurements}

    async def test_concurrent_access(
        self, provider_name: str, concurrent_sessions: int = 10
    ) -> Dict[str, Any]:
        ...
        async def worker_task(worker_id: int, operations: int):
            ...
            async with session_factory() as session:
                for i in range(operations):
                    ...
                    await session.get(f"worker_{worker_id}_item_{i}")
        ...
        tasks = [
            worker_task(i, operations_per_worker) for i in range(concurrent_sessions)
        ]
        ...
        for provider, data in results.items():
            ...
            for op_type in ["set", "get", "delete"]:
                if op_type in data:
                    ...
                    print(
                        f"      Median Latency: {op_data['median_latency_ms']:.3f} ms"
                    )
    ...
    def print_concurrent_results(self, results: List[Dict[str, Any]]):
        ...
        for result in results:
            ...
            print(
                f"   Overall Throughput: {result['overall_ops_per_sec']:,.0f} ops/sec"
            )
        ...
        if "error" not in data:
            for op in ["get", "set"]:
                if op in data:
                    ...
                    print(
                        f"| {provider.title()} | {op.upper()} | {latency:.2f}ms | {throughput:,.0f} ops/sec |"
                    )
