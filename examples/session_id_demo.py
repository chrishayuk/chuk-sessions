...
def demonstrate_basic_generation():
    ...
    for protocol in protocols:
        ...
        try:
            ...
            print(f"  ✓ Generated {len(session_ids)} unique IDs: {len(unique_ids) == len(session_ids)}")
        ...
        if session_id:  # Skip analysis for empty strings
            ...
            if analysis.get('valid', False):
                ...
                if analysis['patterns_detected']:
                    print(f"  ⚠️  Patterns detected: {', '.join(analysis['patterns_detected'])}")
            else:
...
def demonstrate_entropy_analysis():
    ...
    entropy_examples = [
        ...
        },
        {
            ...
            "generator": lambda: generate_secure_session_id("custom", {
                "length": 20,
                "alphabet": "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789",
                "prefix": "med",
                "separator": "-"
    ...
    for example in entropy_examples:
        ...
        try:
            ...
            # Protocol compliance
            compliant_protocols = [p for p, compliant in analysis['protocol_compliance'].items() if compliant]
...
def demonstrate_real_world_usage():
    ...
    scenarios = [
        ...
        },
        {
            ...
            "code": lambda: generate_secure_session_id("custom", {
                "length": 48,
                "alphabet": "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789",
                "prefix": "ak",
                "separator": "_"
            }),
            "validation": lambda sid: validate_session_id_format(sid, "generic", min_entropy_bits=256)
        },
        {
            ...
            "validation": lambda sid: validate_session_id_format(sid, "generic", min_entropy_bits=64, strict_mode=False)
        },
        {
...
def demonstrate_security_comparison():
    ...
    methods = [
        ...
        ("CHUK Sessions High Entropy", generate_secure_session_id("custom", {
            "length": 32,
            "alphabet": "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*",
            "prefix": "secure",
            "separator": "_"
    ...
    for method_name, session_id in methods:
        try:
            ...
            issues = len(analysis.get('patterns_detected', [])) + len(analysis.get('recommendations', []))
            
            # Truncate long session IDs for display
            display_id = session_id[:30] + "..." if len(session_id) > 30 else session_id
            
            print(f"{method_name:<25} {display_id:<35} {entropy:<10.1f} {strength:<12} {issues}")
