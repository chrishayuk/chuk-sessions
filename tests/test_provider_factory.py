...
class TestProviderFactory:
    ...
    async def test_dynamic_provider_loading(self):
        ...
        with patch.dict(os.environ, {'SESSION_PROVIDER': provider_name}):
            with patch('chuk_sessions.provider_factory.import_module', return_value=mock_module) as mock_import:
                result = factory_for_env()
    ...
    async def test_dynamic_provider_factory_is_already_factory(self):
        ...
        with patch.dict(os.environ, {'SESSION_PROVIDER': provider_name}):
            with patch('chuk_sessions.provider_factory.import_module', return_value=mock_module):
                result = factory_for_env()
    ...
    async def test_dynamic_provider_missing_module(self):
        ...
        with patch.dict(os.environ, {'SESSION_PROVIDER': provider_name}):
            with patch('chuk_sessions.provider_factory.import_module', side_effect=ModuleNotFoundError("No module named 'chuk_sessions.providers.nonexistent_provider'")):
                with pytest.raises(ModuleNotFoundError, match="No module named 'chuk_sessions.providers.nonexistent_provider'"):
                    factory_for_env()
    ...
    async def test_dynamic_provider_missing_factory_attribute(self):
        ...
        with patch.dict(os.environ, {'SESSION_PROVIDER': provider_name}):
            with patch('chuk_sessions.provider_factory.import_module', return_value=mock_module):
                with pytest.raises(AttributeError, match="lacks a factory\\(\\) function"):
                    factory_for_env()
        ...
        for provider_name in test_cases:
            with patch.dict(os.environ, {'SESSION_PROVIDER': provider_name}):
                ...
                if provider_name_clean.strip() in ("memory", "mem", "inmemory"):
                    ...
                    with patch('chuk_sessions.provider_factory.import_module', side_effect=ModuleNotFoundError()):
...
class TestProviderFactoryIntegration:
    ...
    async def test_provider_consistency(self):
        ...
        # Test Redis aliases with mocked client
        mock_redis_client = AsyncMock()
        mock_redis_client.setex = AsyncMock()
...
class TestProviderFactoryEdgeCases:
    ...
    async def test_empty_provider_name(self):
        ...
        with patch.dict(os.environ, {'SESSION_PROVIDER': ''}):
            ...
            with patch('chuk_sessions.provider_factory.import_module', side_effect=ModuleNotFoundError("Module not found")):
                with pytest.raises(ModuleNotFoundError):
                    factory_for_env()
    ...
    async def test_circular_import_protection(self):
        ...
        with patch.dict(os.environ, {'SESSION_PROVIDER': provider_name}):
            with patch('chuk_sessions.provider_factory.import_module', side_effect=ImportError("Circular import detected")):
...
class TestProviderFactoryEnvironmentHandling:
    ...
    async def test_module_import_side_effects(self):
        ...
        with patch.dict(os.environ, {'SESSION_PROVIDER': provider_name}):
            with patch('chuk_sessions.provider_factory.import_module', side_effect=import_with_side_effects):
                result = factory_for_env()
