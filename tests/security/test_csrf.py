...
class TestCSRFTokenInfo:
    ...
    def test_csrf_token_info_to_dict(self):
        ...
        expected_keys = {
            "session_id", "timestamp", "token_type", "user_data",
            "age_seconds", "is_valid", "is_expired", "error"
        }
...
class TestBasicCSRFTokens:
    ...
    def test_validate_csrf_token_with_user_data(self):
        ...
        assert validate_csrf_token(token, session_id, secret_key, require_user_data=True) is True
        
        # Token without user data should fail user data requirement
        token_no_data = generate_csrf_token(session_id, secret_key)
        assert validate_csrf_token(token_no_data, session_id, secret_key, require_user_data=True) is False
    ...
    def test_validate_csrf_token_expiration(self):
        ...
        assert validate_csrf_token(token, session_id, secret_key, max_age_seconds=10800) is True
    ...
    def test_validate_csrf_token_malformed(self):
        ...
        # Invalid timestamp
        assert validate_csrf_token("notanumber::signature", session_id, secret_key) is False
...
class TestCSRFTokenExtraction:
    ...
    def test_extract_csrf_token_info_invalid_signature(self):
        ...
        assert info.is_valid is False
        assert "Invalid signature" in info.error  # Changed to check if error contains the string
...
class TestDoubleSubmitTokens:
    ...
    def test_validate_double_submit_token_success(self):
        ...
        # Should validate successfully
        assert validate_double_submit_token(
    ...
    def test_validate_double_submit_token_expired(self):
        ...
        with patch('chuk_sessions.utils.security.csrf.time.time', return_value=8000):
            ...
            # Should pass with longer max age
            assert validate_double_submit_token(
                cookie_token, form_token, session_id, secret_key, max_age_seconds=10000
            ) is True
...
class TestEncryptedTokens:
    ...
    def test_validate_encrypted_csrf_token_success(self):
        ...
        token = generate_encrypted_csrf_token(session_id, secret_key, user_data)
        is_valid, decrypted_data = validate_encrypted_csrf_token(token, session_id, secret_key)
    ...
    def test_validate_encrypted_csrf_token_wrong_session(self):
        ...
        token = generate_encrypted_csrf_token(session_id, secret_key)
        is_valid, decrypted_data = validate_encrypted_csrf_token(token, wrong_session, secret_key)
    ...
    def test_validate_encrypted_csrf_token_wrong_secret(self):
        ...
        token = generate_encrypted_csrf_token(session_id, secret_key)
        is_valid, decrypted_data = validate_encrypted_csrf_token(token, session_id, wrong_secret)
    ...
    def test_validate_encrypted_csrf_token_expired(self):
        ...
        token = generate_encrypted_csrf_token(session_id, secret_key, timestamp=old_timestamp)
        is_valid, decrypted_data = validate_encrypted_csrf_token(token, session_id, secret_key)
...
class TestUtilityFunctions:
    ...
    def test_validate_csrf_token_format(self):
        ...
        assert result is True, f"Encrypted token validation failed for: {encrypted_token}"
...
class TestIntegrationScenarios:
    ...
    def test_api_request_protection_scenario(self):
        ...
        is_valid, api_data = validate_encrypted_csrf_token(api_token, session_id, secret_key)
        assert is_valid is True
...
def test_token_validation_parametrized(token_type, csrf_token_samples, sample_session_data):
    ...
    if token_type == "double_submit":
        ...
        assert validate_double_submit_token(cookie_token, form_token, session_id, secret_key) is True
...
def test_token_expiration_parametrized(max_age, sample_session_data):
    ...
    actual_valid = validate_csrf_token(token, session_id, secret_key, max_age_seconds=max_age)
