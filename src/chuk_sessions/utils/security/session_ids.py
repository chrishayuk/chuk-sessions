...
def generate_secure_session_id(
    ...
    Args:
        protocol: Protocol type (generic, mcp, http, websocket, jwt, uuid, etc.)
    ...
    if protocol == "jwt":
        ...
        length = _calculate_required_length(entropy_bits or 128, 64)  # Base64 alphabet size
    ...
    elif protocol in SESSION_ID_FORMATS:
        format_config = SESSION_ID_FORMATS[protocol].copy()
    else:
        raise ValueError(
...
def _validate_entropy_requirement(length: int, alphabet: str, required_bits: int) -> bool:
...
def _validate_mcp_compliance(session_id: str) -> bool:
    ...
    return (
        PATTERNS["mcp_uuid"].match(session_id) or
        PATTERNS["mcp_custom"].match(session_id)
    )
...
def _validate_protocol_specific(session_id: str, protocol: str, strict_mode: bool) -> bool:
    ...
    if protocol == "mcp":
        ...
        if strict_mode:
            ...
            return PATTERNS["visible_ascii"].match(session_id)
    
    elif protocol == "http":
        # HTTP sessions: URL-safe characters
        if strict_mode:
            return PATTERNS["http_session"].match(session_id)
        else:
            return PATTERNS["http_session_relaxed"].match(session_id.split('-', 1)[-1])
    
    elif protocol == "websocket":
        # WebSocket: similar to HTTP
        return PATTERNS["websocket_session"].match(session_id)
    
    elif protocol == "uuid":
        # UUID format validation
        return PATTERNS["uuid_any"].match(session_id)
    
    elif protocol == "jwt":
        # JWT tokens should be URL-safe base64
        return PATTERNS["base64_urlsafe"].match(session_id)
...
def _validate_entropy(session_id: str, min_entropy_bits: int, strict_mode: bool) -> bool:
    """Validate entropy requirements."""
...
def analyze_session_id_strength(session_id: str) -> Dict[str, Any]:
    ...
    if entropy < DEFAULT_MIN_ENTROPY_BITS:
        recommendations.append(f'Increase entropy to at least {DEFAULT_MIN_ENTROPY_BITS} bits')
    if diversity < 0.3:
